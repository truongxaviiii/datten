    ...
    <Link to={mediaType !== 'people' ? routesGen.mediaDetail(mediaType, media.mediaId || media.id) : routesGen.person(media.id)}>
      <Box
        sx={{
          ...
          '&:hover .media-back-drop, &:hover .media-btn': { opacity: 1 },
          color: 'primary.contrastText',
          '&:hover .media-btn': {
            backgroundColor: 'transparent', // Đảm bảo màu nền khi hover là trong suốt
            color: 'GrayText', // Đảm bảo màu chữ không đổi
          },
        }}
      >
        {/* movie or tv item */}
          ...
          <>
            ...
            <Button
              ...
              sx={{
                ...
                '& .MuiButton-startIcon svg': { fontSize: '4.7rem', color: 'white' },
                '& .MuiSvgIcon-root': {
                  fontSize: '2rem', // Kích thước icon ban đầu
                  transition: 'color 0.3s ease', // Hiệu ứng chuyển màu mượt mà
                },
                '&:hover': {
                  backgroundColor: 'transparent', // Màu nền khi hover, nếu cần
                  color: 'gray', // Màu của text và icon khi hover
                },
